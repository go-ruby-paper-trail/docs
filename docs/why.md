...
`go-ruby-paper-trail/paper-trail` reimplements Ruby's `paper-trail` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-paper-trail/paper-trail` directly, with no Ruby runtime;
...
Because `paper-trail` is pure Go with cgo disabled, it also compiles to **WebAssembly**
